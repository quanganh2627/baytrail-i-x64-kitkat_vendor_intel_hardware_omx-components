...
#define WV_MAX_PACKETS_IN_FRAME 22 /* 20*64K=1.3M, max frame size */
...
#define WV_DMA_IN_SWAP  true
#define WV_DMA_OUT_SWAP false
#define WV_XCRIPT_DMA_OUT_SWAP (NO_SWAP)
...
typedef enum {
    ...
    WV_FAIL_CTR_XCRIPT_FAILURE,
    WV_FAIL_DMA_XCRIPT_FAILURE,
    ...
    WV_FAIL_SGL_FULL,
    WV_FAIL_SGL_SIZE_TOO_BIG,
    WV_FAIL_NOT_ENOUGH_SGL,
    WV_FAIL_SGL_INVALID,
    WV_FAIL_HANDLE_INVALID_METADATA,
    WV_FAIL_HANDLE_INVALID_SRC,
    WV_FAIL_HANDLE_INVALID_DEST,
    WV_FAIL_HANDLE_INVALID_HEADER,
    WV_FAIL_HANDLE_TOO_SMALL_METADATA,
    WV_FAIL_HANDLE_TOO_SMALL_SRC,
    WV_FAIL_HANDLE_TOO_SMALL_DEST,
    WV_FAIL_HANDLE_TOO_SMALL_HEADER,
    WV_FAIL_CONNECTION_STATUS_CHECK_OVERDUE,
    WV_FAIL_KEYBOX_PROVISIONING_CEK_INVALID,
    WV_FAIL_KEYBOX_PROVISIONING_CEK_NOT_AVAILABLE,
    WV_FAIL_KEYBOX_PROVISIONING_AFTER_EOM,
    WV_FAIL_KEYBOX_PROVISIONING_UNKNOWN_ERROR_1,
    WV_FAIL_KEYBOX_PROVISIONING_UNKNOWN_ERROR_2,
    WV_FAIL_KEYBOX_PROVISIONING_UNKNOWN_ERROR_3,
    WV_FAIL_KEYBOX_PROVISIONING_UNKNOWN_ERROR_4,
    WV_FAIL_KEYBOX_PROVISIONING_UNKNOWN_ERROR_5,
    WV_FAIL_KEYBOX_PROVISIONING_UNKNOWN_ERROR_6,
    WV_FAIL_KEYBOX_PROVISIONING_UNKNOWN_ERROR_7,
    WV_FAIL_INVALID_ICV,
    WV_INVALID_STREAM_ID,
    WV_FAIL_INVALID_PAVP_MEMORY_MODE
} wv_heci_status;
...
typedef enum {
    ...
    wv_title_completed,
    wv_uninit_dma,
    wv_get_display_nonce,
    wv_process_connection_status,
    wv_heci_end = wv_process_connection_status,
    ...
    wv_dbg_generate_rsa_keypair,
    wv_dbg_get_rsa_public_key,
    wv_heci_dbg_end = wv_dbg_get_rsa_public_key,
    ...
    wv_uninit_dma_rsp,
    wv_get_display_nonce_rsp,
    wv_process_connection_status_rsp,
    ...
    wv_dbg_generate_rsa_keypair_rsp,
    wv_dbg_get_rsa_public_key_rsp,
} wv_heci_command_id;
...
typedef struct {
    PAVP_CMD_HEADER  Header;
    uint32_t         num_sgl_entries;
    wv_sgl_entry     sgl_list[320];
} wv_init_dma_in;
...
typedef struct {
    ...
    uint32_t         metadata_handle;
    uint32_t         frame_handle;
    uint32_t         parsed_header_handle;
...
typedef struct {
    ...
    uint32_t         num_of_packets;//<=20
    BOOL             is_frame_not_encrypted;
    uint32_t         src_mrd_handle;
    uint32_t         dest_mrd_handle;
    uint32_t         metadata_mrd_handle;
...
/*wv_get_display_nonce*/
typedef PAVP_CMD_NODATA wv_get_display_nonce_in;

typedef struct {
    PAVP_CMD_HEADER  Header;
    uint32_t         nonce;
} wv_get_display_nonce_out;

/*wv_process_connection_status*/
typedef struct {
    PAVP_CMD_HEADER  Header;
    uint8_t  connection_status[32];
} wv_process_connection_status_in;

typedef struct {
    PAVP_CMD_HEADER  Header;
    BOOL  HdcpOk;
} wv_process_connection_status_out;
...
typedef struct {
    ...
    uint32_t         sgl_8_size;
    uint32_t         sgl_9_size;
    uint32_t         sgl_10_size;
    uint32_t         sgl_11_size;
    uint32_t         sgl_12_size;
    uint32_t         sgl_13_size;
    uint32_t         sgl_14_size;
    uint32_t         sgl_15_size;
    uint32_t         sgl_16_size;
    uint32_t         sgl_17_size;
    ...
    uint32_t         time_parse;
    uint32_t         time_encrypt;
    uint32_t         time_dma_out;
///
} wv_dbg_get_diag_out;
...
#define WV_RSA_N_SIZE_BYTES   256
#define WV_RSA_P_SIZE_BYTES   (WV_RSA_N_SIZE_BYTES/2)
#define WV_RSA_E_SIZE_BYTES   4

/*wv_dbg_generate_rsa_keypair*/
typedef PAVP_CMD_NODATA wv_dbg_generate_rsa_keypair_in;
typedef struct {
    PAVP_CMD_HEADER  Header;
    uint8_t          Gkey_encrypted_p[WV_RSA_P_SIZE_BYTES];
    uint8_t          n[WV_RSA_N_SIZE_BYTES];
    uint8_t          e[WV_RSA_E_SIZE_BYTES];
} wv_dbg_generate_rsa_keypair_out;

/*wv_dbg_get_rsa_public_key*/
typedef PAVP_CMD_NODATA wv_dbg_get_rsa_public_key_in;
typedef struct {
    PAVP_CMD_HEADER  Header;
    uint8_t          n[WV_RSA_N_SIZE_BYTES];
    uint8_t          e[WV_RSA_E_SIZE_BYTES];
} wv_dbg_get_rsa_public_key_out;
#endif
